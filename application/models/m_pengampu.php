...
class M_Pengampu extends CI_Model{
	...
	function num_page($semester_tipe,$tahun_akademik){
		...
		$rs = $this->db->query(
							    "SELECT CAST(COUNT(*) AS CHAR(4)) as cnt FROM pengampu a LEFT JOIN matakuliah b ON a.kode_mk = b.kode LEFT JOIN dosen c
								ON a.kode_dosen = c.kode WHERE b.semester%2=$semester_tipe AND a.tahun_akademik = '$tahun_akademik' ORDER BY b.nama,a.kelas");
		return $rs->row()->cnt;
	...
	function cek_for_update($kode,$kode_mk,$kode_dosen,$kelas,$tahun_akademik,$kode){		
		$rs = $this->db->query(
							   "SELECT CAST(COUNT(*) AS CHAR(1)) as cnt FROM pengampu WHERE kode_mk='$kode_mk' 
							   AND kode_dosen=$kode_dosen AND kelas = '$kelas' AND tahun_akademik='$tahun_akademik' AND kode <> $kode");
		return $rs->row()->cnt;
	}
	
	function cek_for_insert($kode_mk,$kode_dosen,$kelas,$tahun_akademik,$kode){		
		$rs = $this->db->query(
							   "SELECT CAST(COUNT(*) AS CHAR(1)) as cnt FROM pengampu WHERE kode_mk='$kode_mk' AND kode_dosen=$kode_dosen AND 
							   kelas = '$kelas' AND tahun_akademik='$tahun_akademik' ");
		return $rs->row()->cnt;
	}
