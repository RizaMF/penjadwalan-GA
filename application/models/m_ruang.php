...
class M_Ruang extends CI_Model{
	...
	function cek_for_update($nama,$kode){
		$rs = $this->db->query("SELECT CAST(COUNT(*) AS CHAR(1)) as cnt FROM ruang WHERE nama='$nama' AND kode <> $kode");
		return $rs->row()->cnt;
	}
	
	function cek_for_insert($nama){
		$rs = $this->db->query("SELECT CAST(COUNT(*) AS CHAR(1)) as cnt FROM ruang WHERE nama='$nama'");
		return $rs->row()->cnt;
	}
