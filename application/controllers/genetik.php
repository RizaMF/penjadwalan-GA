...
class Genetik extends CI_Controller
{
    ...
    public function AmbilData()
    {
        ...
        //ruang teori
        $rs_RuangReguler = $this->db->query("SELECT kode FROM ruang WHERE jenis = '$this->TEORI'");
        $i               = 0;
        ...
        //ruang praktek/lab
        $rs_Ruanglaboratorium = $this->db->query("SELECT kode FROM ruang WHERE jenis = '$this->LABORATORIUM'");
        $i                    = 0;
        ...
        //ruang proyek /TA
        $rs_Ruangproyek = $this->db->query("SELECT kode FROM ruang WHERE jenis = '$this->PROYEK'");
        $i                    = 0;
        foreach ($rs_Ruangproyek->result() as $data) {
            $this->ruangProyek[$i] = intval($data->kode);
            $i++;
        }
        
        //ruang Bahasa Inggris
        $rs_Ruangbahasa = $this->db->query("SELECT kode FROM ruang WHERE jenis = '$this->BAHASA'");
        $i                    = 0;
        foreach ($rs_Ruangbahasa->result() as $data) {
            $this->ruangBahasa[$i] = intval($data->kode);
            $i++;
        }
        //var_dump($this->ruangBahasa);
        //exit(0);
